...
const normalize = (row: Record<string, unknown>, requester?: RequesterProfile | null) => ({
  id: row.id,
  userId: row.user_id,
  userName: requester?.full_name?.trim() || requester?.email?.trim() || 'Utilisateur',
  userEmail: requester?.email ?? null,
  userPhone: requester?.phone ?? null,
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === 'GET') {
    ...
    const profilesById = new Map<string, RequesterProfile>();
    if (userIds.length > 0) {
      ...
      for (const item of profiles ?? []) {
        profilesById.set(String(item.id), item as RequesterProfile);
      }
    }
    return json(rows.map((row) => normalize(row, profilesById.get(String(row.user_id)))));
  }
  ...
  if (req.method === 'PATCH') {
    ...
    const { data: requester } = await supabase
      ...
      .eq('id', data.user_id)
      .maybeSingle();
    return json(normalize(data as Record<string, unknown>, requester));
  }
  ...
  if (
    departureAddress.length < 3 ||
    arrivalAddress.length < 3 ||
    !/^\d{4}-\d{2}-\d{2}$/.test(movingDate)
  ) {
    return json({ error: 'Adresses et date valides requis' }, 400);
  }
  if (
    !Number.isFinite(estimatedVolume) ||
    estimatedVolume <= 0 ||
  ...
  const { data: requester } = await supabase
    ...
    .eq('id', user.id)
    .maybeSingle();
  return json(normalize(data as Record<string, unknown>, requester), 201);
});
