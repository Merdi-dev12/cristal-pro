...
const SERVICE_FIELDS = {
  ...
  maintenance: {
    allowed: ["intervention_type", "urgency", "property_address", "preferred_date", "access_details"],
...
Deno.serve(async (request: Request) => {
  ...
  if (description.length < 20) return json({ error: "La description doit contenir au moins 20 caractères" }, 400);
...
function validateDetails(type: ServiceType, input: unknown): { details: Details } | { error: string } {
  ...
  if (JSON.stringify(details).length > 10_000) return { error: "Les informations du dossier sont trop volumineuses" };
  return { details };
}
