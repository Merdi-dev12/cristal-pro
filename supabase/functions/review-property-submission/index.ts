...
Deno.serve(async (req: Request) => {
  ...
  const id = clean(body.submission_id, 80);
  const action = clean(body.action, 30);
  const message = clean(body.message, 4000);
  if (
    !id ||
    !['approve', 'reject', 'request_details'].includes(action) ||
    ((action === 'reject' || action === 'request_details') && !message)
  )
    return json({ error: 'Action ou message invalide.' }, 400);
  ...
  if (submission.status === 'publiée')
    return json({ error: 'Cette annonce est déjà publiée.' }, 409);
  let status = '';
  let kind = '';
  let notification = '';
  ...
  if (action === 'approve') {
    const { data: property, error } = await supabase
      ...
      .insert({
        ...
        price_suffix: submission.type === 'location' ? '/mois' : null,
    ...
    kind = 'approved';
    notification =
      message || 'Votre annonce a été validée et est maintenant visible par tous les utilisateurs.';
  ...
  const { error: updateError } = await supabase
    ...
    .update({
      ...
      rejection_reason: action === 'reject' ? notification : null,
  ...
  const { error: decisionError } = await supabase
    .from('property_submission_decisions')
    .insert({ submission_id: id, status, reason: notification, decided_by: user.id });
  if (decisionError) return json({ error: decisionError.message }, 400);
  const { error: notificationError } = await supabase
    .from('property_submission_notifications')
    .insert({ submission_id: id, recipient_id: submission.owner_id, kind, message: notification });
