      ...
      <header class="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <p class="mt-3 max-w-2xl text-sm leading-6 text-rheo-muted">
            Recherchez, filtrez et ouvrez chaque annonce dans une page complète.
          </p>
        </div>
        <a
              ...
              <a
                ...
                /><span class="min-w-0 flex-1"
                  ...
                  ><span class="mt-1 block truncate text-sm text-rheo-muted"
                    >📍 {{ item.location }} · {{ item.address }}</span
                  ><span class="mt-2 block text-xs text-[#7a8375]"
                    >{{ item.price | number }} USD{{ item.priceSuffix }} · {{ item.surface }} m² ·
                    {{ item.ownerName }}</span
                  ></span
                ><span class="hidden font-semibold text-[#657b18] sm:block">Gérer →</span></a
              >
            } @empty {
