...
export class PropertySearchService {
  ...
  fromQueryParams(params: ParamMap): PropertySearchFilters {
    return {
      ...
      category: this.readUnion(
        params.get('category'),
        ['all', 'maison', 'appartement', 'residence', 'immeuble', 'terrain'],
        'all',
      ),
      ...
      sort: this.readUnion(
        params.get('sort'),
        ['recent', 'price-asc', 'price-desc', 'surface-desc'],
        'recent',
      ),
    };
  }
  ...
  filterLocal(properties: Property[], filters: PropertySearchFilters): Property[] {
    ...
    const result = properties.filter((property) => {
      const matchesQuery =
        !query ||
        this.normalize(`${property.title} ${property.location} ${property.description}`).includes(
          query,
        );
      const matchesType = filters.type === 'all' || property.type === filters.type;
      const matchesCategory = filters.category === 'all' || property.category === filters.category;
      const matchesLocation =
        !location || this.normalize(`${property.location} ${property.address}`).includes(location);
